<template>
  ...
  export default {
    ...
    computed: {
      filteredDishes() {
        let filtered = this.allDishes.filter(dish => dish.tip === "pice");
    ...
    methods: {
      ...
      dishMatchesSearch(dish) {
        ...
        return serbianName.includes(searchText) || englishName.includes(searchText) || dish.cenaV.toString().includes(searchText);
      }
    }
  };
  </script>
